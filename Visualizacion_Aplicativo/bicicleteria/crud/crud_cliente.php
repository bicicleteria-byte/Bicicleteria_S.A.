stionar productos · PHP
<?php
include("../conexion.php");
 
// Métricas
$sql_total   = "SELECT COUNT(*) AS total FROM producto";
$sql_stock   = "SELECT COUNT(*) AS bajo FROM producto WHERE stock <= 3";
$sql_valor   = "SELECT SUM(precio * stock) AS valor FROM producto";
 
$total  = mysqli_fetch_assoc(mysqli_query($conexion, $sql_total))['total'];
$bajo   = mysqli_fetch_assoc(mysqli_query($conexion, $sql_stock))['bajo'];
$valor  = mysqli_fetch_assoc(mysqli_query($conexion, $sql_valor))['valor'];
 
// Búsqueda
$buscar = isset($_GET['q']) ? mysqli_real_escape_string($conexion, $_GET['q']) : '';
$sql = $buscar
    ? "SELECT * FROM producto WHERE nombre LIKE '%$buscar%' OR id_producto LIKE '%$buscar%'"
...
<aside class="sidebar">
  ...
  <a href="gestionar_productos.php" class="nav-item active"><i class="ti ti-package"></i><span>Empleados</span></a>
  <a href="gestionar_productos.php" class="nav-item active"><i class="ti ti-package"></i><span>Registrar</span></a>
  <a href="gestionar_productos.php" class="nav-item active"><i class="ti ti-package"></i><span>Calcular horas trabajadas</span></a> 
  <a href="gestionar_productos.php" class="nav-item active"><i class="ti ti-package"></i><span>Historial de horas trabajadas</span></a>
...
<main class="main">
  ...
  <div class="stats">
    <div class="stat-card">
      <div class="stat-label"><i class="ti ti-package"></i> Total productos</div>
      <div class="stat-val"><?php echo $total; ?></div>
    </div>
    <div class="stat-card danger">
      <div class="stat-label"><i class="ti ti-alert-triangle"></i> Stock bajo (≤3)</div>
      <div class="stat-val"><?php echo $bajo; ?></div>
    </div>
    <div class="stat-card">
      <div class="stat-label"><i class="ti ti-currency-dollar"></i> Valor inventario</div>
      <div class="stat-val">$<?php echo number_format($valor, 0, ',', '.'); ?></div>
    </div>
  </div>
  ...
  <div class="tabla-wrap">
    <div class="tabla-header">
      <span>Todos los productos <?php if($buscar) echo "— <em style='font-weight:400;color:#6B7280'>\"$buscar\"</em>"; ?></span>
      ...
      <form class="search-form" method="GET" action="">
        ...
        <?php if($buscar): ?>
          <a href="gestionar_productos.php" class="btn-editar" style="border-color:#E5E7EB">
        ...
        <?php if(mysqli_num_rows($resultado) === 0): ?>
          <tr class="empty-row">
            <td colspan="6">
              ...
              No se encontraron productos<?php echo $buscar ? " para \"$buscar\"" : ''; ?>
            </td>
          </tr>
        <?php else: ?>
          <?php while($fila = mysqli_fetch_assoc($resultado)): ?>
            <?php
              // Badge según stock
              if ($fila['stock'] == 0) {
                $badge = 'badge-out'; $label = 'Sin stock'; $icon = 'ti-circle-x';
              } elseif ($fila['stock'] <= 3) {
                $badge = 'badge-low'; $label = 'Stock bajo'; $icon = 'ti-alert-triangle';
              } else {
                $badge = 'badge-ok';  $label = 'Disponible'; $icon = 'ti-circle-check';
              }
            ?>
            <tr>
              <td class="id-cell">#<?php echo str_pad($fila['id_producto'], 3, '0', STR_PAD_LEFT); ?></td>
              ...
              <td>
                <div class="actions">
                  <a href="editar_producto.php?id=<?php echo $fila['id_producto']; ?>" class="btn-editar">
